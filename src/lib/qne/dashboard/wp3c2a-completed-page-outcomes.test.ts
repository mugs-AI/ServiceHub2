...
import { deriveOutcome } from "@/lib/qne/service-jobs/wp3b-followup";
...
describe("WP3C-2A Completed page outcomes", () => {
  const dbBlock = route.slice(
    route.indexOf("if (dbPaged)"),
    route.indexOf("return Response.json", route.indexOf("if (dbPaged)")) + 600,
  );
  ...
  it("loader accepts an optional bounded ID set and keeps default behaviour", () => {
    expect(loader).toMatch(
      /loadCompletionOutcomes\(\s*tenantCode: string,\s*jobIds\?: readonly string\[\]/,
    );
