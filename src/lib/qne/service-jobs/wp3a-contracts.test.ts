...
describe("candidate migration", () => {
  ...
  it("ships candidate types instead of editing the generated Supabase types", () => {
    ...
    // The WP3A migration has since been applied and authorised, so the
    // generated types now describe the real schema. They are only ever
    // regenerated by the platform after an authorised migration — never
    // hand-edited to pretend an unapplied migration exists.
    const generated = read("src", "integrations", "supabase", "types.ts");
    expect(generated).toContain("service_job_reopen_requests");
    expect(generated).toContain("latest_customer_ref_no");
  });
